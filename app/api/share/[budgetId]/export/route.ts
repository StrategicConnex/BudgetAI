...
const log = createLogger('API/share-export');

export const maxDuration = 60;

/**
 * Public export route for shared budgets.
 * GET /api/share/[budgetId]/export?format=pdf|html
 *
 * No auth required — budget must have status 'ready' or 'exported'
 * to be accessible (enforced by RLS policy 004_public_share_policy).
 */
export const GET = withRateLimit(async (request: NextRequest) => {
  try {
    ...
    // /api/share/[budgetId]/export -> ['', 'api', 'share', budgetId, 'export']
    ...
    if (format !== 'pdf' && format !== 'html') {
      return NextResponse.json(
        { error: 'format debe ser "pdf" o "html"' },
        { status: 400 }
      );
    }

    // Fetch budget without auth — relies on RLS public read policy
    ...
    const filename = budgetData.numero || budgetData.titulo || 'presupuesto';

    if (format === 'pdf') {
      const pdfBuffer = await generatePDF(budgetData);
      return new NextResponse(new Uint8Array(pdfBuffer), {
        status: 200,
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="presupuesto-${filename}.pdf"`,
          'Content-Length': String(pdfBuffer.length),
        },
      });
    }

    // HTML export — inline the existing generateHTML function from /api/export/html/route
    const htmlContent = generateHTML(budgetData);
    return new NextResponse(htmlContent, {
      status: 200,
      headers: {
        'Content-Type': 'text/html; charset=utf-8',
        'Content-Disposition': `attachment; filename="presupuesto-${filename}.html"`,
      },
    });
  } catch (error) {
    log.error('Error exportando presupuesto compartido', error instanceof Error ? error : undefined);
    return NextResponse.json(
      { error: 'Error al exportar el presupuesto' },
      { status: 500 }
    );
  }
}, { limit: 'export', identifier: 'ip' });

// ── HTML generator (reused from /api/export/html) ──────
...
<head>
  ...
  <style>
    ...
    body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background-color: #f3f4f6; color: var(--text-main); margin: 0; padding: 40px 20px; line-height: 1.5; }
    .document { max-width: 800px; margin: 0 auto; background: var(--bg-main); border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); overflow: hidden; }
    .header { padding: 40px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: flex-start; background: linear-gradient(135deg, rgba(92,98,236,0.05), rgba(124,58,237,0.05)); }
    .badge { display: inline-block; padding: 4px 12px; background: rgba(92,98,236,0.1); color: var(--primary); border-radius: 9999px; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 16px; }
    ...
    @media print { body { background: white; padding: 0; } .document { box-shadow: none; border-radius: 0; max-width: 100%; } }
  </style>
</head>
<body>
  <div class="document">
