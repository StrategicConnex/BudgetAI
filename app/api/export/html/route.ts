...
export async function POST(req: Request) {
  try {
    const { budget } = await req.json() as { budget: BudgetData };
    ...
    return new NextResponse(htmlContent, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="presupuesto-${budget.numero || Date.now()}.html"`,
      },
    });
        ...
        ${items.map(item => `
          <tr>
            <td>
              ...
              ${item.observaciones ? `<div class="item-obs">* ${item.observaciones}</div>` : ''}
            </td>
            <td>${item.unidad}</td>
...
<head>
  ...
  <title>Presupuesto ${b.numero}</title>
  <style>
    :root {
      --primary: #5c62ec;
      --text-main: #111827;
    ...
    .client-detail, .condition-detail {
      color: var(--text-muted);
      font-size: 14px;
      margin-bottom: 2px;
    }
    ...
    .total-final {
      ...
      align-items: center;
      padding: 16px;
      background: linear-gradient(135deg, #5c62ec, #7c3aed);
      color: white;
      border-radius: 8px;
      margin-top: 16px;
    }
...
<body>
  <div class="document">
    <div class="header">
      <div>
        <div class="badge">${b.categoria}</div>
        <h1>${b.titulo}</h1>
        <p class="header-desc">${b.descripcionGeneral}</p>
      </div>
      <div class="meta-info">
        <div class="budget-number">${b.numero}</div>
    ...
    <div class="parties">
      <div class="party-col">
        ...
        <div class="client-name">${b.cliente.nombre}</div>
        ${b.cliente.empresa ? `<div class="client-detail">${b.cliente.empresa}</div>` : ''}
        ${b.cliente.email ? `<div class="client-detail">${b.cliente.email}</div>` : ''}
        ${b.cliente.cuit ? `<div class="client-detail">CUIT: ${b.cliente.cuit}</div>` : ''}
      </div>
      <div class="party-col">
        <div class="party-label">Condiciones</div>
        <div class="condition-row">
          <span class="total-label">Validez:</span>
          <span class="condition-value">${b.condiciones.validezDias} días</span>
        </div>
        <div class="condition-row">
          <span class="total-label">Pago:</span>
          <span class="condition-value">${b.condiciones.formaPago}</span>
        </div>
        ${b.condiciones.notas ? `<div class="condition-detail" style="margin-top: 8px; font-size: 12px;">${b.condiciones.notas}</div>` : ''}
      </div>
    </div>
